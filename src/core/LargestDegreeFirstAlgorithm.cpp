...
namespace {
std::mutex mtx;
void LargestDegreeFirstWorker(std::vector<Vertex>& vertices, const std::unordered_set<int>& U,
...
void LargestDegreeFirstAlgorithm::colorGraph(std::vector<Vertex>& vertices) {
    ...
    while (!U.empty()) {
        ...
        for (int i = 0; i < this->_numWorkers; i++) {
            ...
            workers.emplace_back(LargestDegreeFirstWorker, std::ref(vertices), std::ref(U), r_p,
                                 U_begin, U_end, std::ref(i_set));
        }
        for (auto& t : workers) {
            t.join();
        }
